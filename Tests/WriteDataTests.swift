...
class WriteDataTests: UZKArchiveTestCase {
    ...
    func testWriteData() {
        ...
        for (index, testFilePath) in testFilePaths.enumerated() {
            ...
            do {
                let props = ZipFileProperties(testFilePath)
                props.timestamp = testDates[index]
                try archive.write(fileData!, props: props)
    ...
    func testWriteData_deprecatedOverload() {
        let testFilePaths = [String](nonZipTestFilePaths).sorted(by: <)
        let testDates = [
            UZKArchiveTestCase.dateFormatter().date(from: "12/20/2014 9:35 AM"),
            UZKArchiveTestCase.dateFormatter().date(from: "12/21/2014 10:00 AM"),
        ...
        for (index, testFilePath) in testFilePaths.enumerated() {
            ...
            do {
                try archive.deprecatedWrite(fileData!, filePath: testFilePath, fileDate: testDates[index],
    ...
    func testWriteData_Unicode_deprecatedOverload() {
        let testFilePaths = [String](nonZipUnicodeFilePaths).sorted(by: <)
        let testDates = [
            UZKArchiveTestCase.dateFormatter().date(from: "12/20/2014 9:35 AM"),
            UZKArchiveTestCase.dateFormatter().date(from: "12/21/2014 10:00 AM"),
    ...
    func testWriteData_Overwrite() {
        ...
        for i in 0..<testFilePaths.count {
            ...
            do {
                let props = ZipFileProperties(testFilePaths[i])
                props.timestamp = testDates[x]
                try archive.write(testFileData[x], props: props)
            } catch let error as NSError {
    ...
    func testWriteData_Overwrite_deprecatedOverload() {
        let testFilePaths = [String](nonZipTestFilePaths).sorted(by: <)
        let testDates = [
            UZKArchiveTestCase.dateFormatter().date(from: "12/20/2014 9:35 AM"),
            UZKArchiveTestCase.dateFormatter().date(from: "12/21/2014 10:00 AM"),
        ...
        for i in 0..<testFilePaths.count {
            ...
            do {
                try archive.deprecatedWrite(testFileData[x], filePath: testFilePaths[i],
    ...
    func testWriteData_Overwrite_Unicode_deprecatedOverload() {
        let testFilePaths = [String](nonZipUnicodeFilePaths).sorted(by: <)
        let testDates = [
            UZKArchiveTestCase.dateFormatter().date(from: "12/20/2014 9:35 AM"),
            UZKArchiveTestCase.dateFormatter().date(from: "12/21/2014 10:00 AM"),
    ...
    func testWriteData_NoOverwrite_deprecatedOverload() {
        let testFilePaths = [String](nonZipTestFilePaths).sorted(by: <)
        let testDates = [
            UZKArchiveTestCase.dateFormatter().date(from: "12/20/2014 9:35 AM"),
            UZKArchiveTestCase.dateFormatter().date(from: "12/21/2014 10:00 AM"),
    ...
    func testWriteData_MultipleWrites_deprecatedOverload() {
        let testArchiveURL = tempDirectory.appendingPathComponent("MultipleDataWriteTest.zip")
        let testFilename = nonZipTestFilePaths.first!
        let testFileURL = testFileURLs[testFilename] as! URL
        let testFileData = try! Data(contentsOf: testFileURL)
        ...
        for _ in 0..<100 {
            do {
                try archive.deprecatedWrite(testFileData, filePath: testFilename, fileDate: nil,
                                            compressionMethod: .default, password: nil)
            } catch let error as NSError {
                XCTFail("Error writing to file \(testFileURL): \(error)")
            }
...
//    }
    ...
    func testWriteData_DefaultDate_deprecatedOverload() {
        let testArchiveURL = tempDirectory.appendingPathComponent("DefaultDateWriteTest.zip")
        let testFilename = nonZipTestFilePaths.first!
        let testFileURL = testFileURLs[testFilename] as! URL
        let testFileData = try! Data(contentsOf: testFileURL)
    ...
    func testWriteData_ExternalVolume() {
        ...
        do {
            try archive.write(newTextData!, filePath: textFileName)
        } catch let error {
    ...
    func testWriteData_ExternalVolume_deprecatedOverload() {
        // Create a simple zip file
        let tempDirURL = URL(fileURLWithPath: self.randomDirectoryName())
        let textFileName = "testWriteData_ExternalVolume.txt"
        let textFileURL = tempDirURL.appendingPathComponent(textFileName)
        ...
        var writeSuccessful = true
        do {
            try archive.deprecatedWrite(newTextData!, filePath: textFileName, fileDate: nil,
